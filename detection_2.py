...
# ── per-camera latest frame store (thread-safe) ──────────
...
def camera_reader(cam_name, url):
    """Each camera runs in its own thread, always reads latest frame."""
    cap = cv2.VideoCapture(url, cv2.CAP_FFMPEG)
    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # keep buffer tiny
    while True:
        ret, frame = cap.read()
        if not ret:
            continue  # keep trying, don't crash
        with frame_locks[cam_name]:
            latest_frames[cam_name] = frame  # always overwrite with latest
...
# ── skip frame counter (run yolo every N frames) ─────────
YOLO_EVERY = 3  # run YOLO every 3 frames, show all frames
frame_counter = 0
...
while True:
    ...
    for cam_name in CAM_NAMES:
        ...
        frame = frame.copy()  # don't modify shared frame
